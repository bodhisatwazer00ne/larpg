...
export const ArenaScreen: React.FC<ArenaScreenProps> = ({
  ...
  // Track battles already processed or exited in this session to prevent re-entry loops
  ...
  useEffect(() => {
    const unsubTrainers = subscribeToPublicTrainers(
      (trainers) => {
        // Filter out current user by both userId and username, and deduplicate rivals
        const cleanPlayerName = user.username.toLowerCase().trim();
        const rivals = trainers.filter(
          (t) => t.userId !== user.id && t.username.toLowerCase().trim() !== cleanPlayerName
        );
  ...
  useEffect(() => {
    const unsubOutgoing = subscribeToOutgoingChallenges(user.id, (outgoingList) => {
      ...
      if (accepted && accepted.battleId && !activeBattleId) {
        ...
        processedBattlesRef.current.add(accepted.battleId);
    ...
    return (
      <div className="space-y-3 select-none">
        ...
        <div className="flex items-center justify-between bg-[#181425] border-2 border-[#120e1d] px-3 py-1.5 text-[#f4eee3]">
          ...
          <button
            onClick={() => {
              if (activeBattleId) processedBattlesRef.current.add(activeBattleId);
