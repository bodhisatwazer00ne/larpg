...
export async function syncPublicTrainerProfile(
  ...
  try {
    const payload: PublicTrainer = {
      ...
      xp: user.xp ?? 0,
      attributes: user.attributes ?? { str: 10, int: 10, end: 10, res: 10, dis: 10, wil: 10 },
...
function emailToSafeAccountId(email: string): string {
    ...
    if (cleanEmail) {
      ...
      setDoc(
        ...
        sanitizeForFirestore({
          uid: targetId,
          email: cleanEmail,
          displayName: state.user.username || cleanEmail.split('@')[0],
          updatedAt: new Date().toISOString(),
        }),
        { merge: true }
      ).catch(() => {});
